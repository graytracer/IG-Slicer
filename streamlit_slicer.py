...
class StreamlitImageSlicer:
    def __init__(self):
        ...
        st.set_page_config(
            page_title="IG-Slicer 25'",
            layout="wide",
            initial_sidebar_state="expanded"  # Always show sidebar on start
        )
        ...
        if 'redirect_shown' not in st.session_state:
            st.session_state.redirect_shown = False

    def create_ui(self):
        # Add redirect to new website
        if not st.session_state.redirect_shown:
            st.session_state.redirect_shown = True
            redirect_html = """
            <div style="background-color: #f8f9fa; padding: 20px; border-radius: 10px; text-align: center; margin: 20px 0;">
                <h1 style="color: #4CAF50;">IG-Slicer has moved to a new home!</h1>
                <h2 style="color: #2196F3;">www.igslicer.site</h2>
                <p style="font-size: 18px; margin: 15px 0;">You will be redirected to our new website in 4 seconds...</p>
            </div>
            <script>
                setTimeout(function() {
            ...
            # Just display a simplified version of the UI while waiting for redirect
            st.title("New IG-Slicer")
            st.markdown("""
                ## We've upgraded! Visit www.igslicer.site for the latest version.
                The new website offers improved features and better performance.
            """)
            
            # Early return to prevent loading the rest of the UI
            return
        ...
        if 'processed_image' not in st.session_state or st.session_state.processed_image is None:
            st.markdown("""
                ...
                **⚠️ Upload the sliced images in 4:5 ratio on instagram     (Not 1:1)**
